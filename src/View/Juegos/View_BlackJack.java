...
public class View_BlackJack extends javax.swing.JFrame {
    ...
    public View_BlackJack() {
        initComponents();
        cargarImagenesReverso();
        this.blackJack = new BlackJack(new Baraja());
    }
    ...
    private void label_pedirMouseClicked(java.awt.event.MouseEvent evt) {
        ...
        if (contador < 4) {
            if (contador == 0 || contador == 2) {
                ...
                blackJack.darCartaJugador();
                String rutaImagen2 = blackJack.getRutaImagenCartaJugador();
                actualizarImagenCartaJugador2(rutaImagen2);

                blackJack.darCartaCrupier();
                String rutaImagenC1 = blackJack.getRutaImagenCartaBanca();
                actualizarImagenCartaCrupier1(rutaImagenC1);

                blackJack.darCartaCrupier();
                String rutaImagenC2 = blackJack.getRutaImagenCartaBanca();
        ...
        if (contador == 4) {
            if (blackJack.getValorJugador() > blackJack.getValorBanca()) {
                ...
                dispose();
                new Victoria();
            } else if (blackJack.getValorJugador() < blackJack.getValorBanca()) {
                dispose();
                new Derrota();
            } else {
                transpasojugador = blackJack.getValorJugador();
